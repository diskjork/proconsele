...
$this->breadcrumbs=array(
	'Movimientos de caja'=>array('index'),
	'Vista',
);
?>
<?php
...
$this->nav=$this->checkCajas($model);
...
<?php if(isset($this->nav)){
		if($this->nav !== 0){
		 echo "	<h5 class='well well-small'>MOVIMIENTOS DE CAJA</h5><br>";
		...
		for($i=0;$i<count($this->nav);$i++){
			$bancoTem=$bancoVar.$this->nav[$i]['idcaja'];
			$$bancoTem=false;
			if($bancoIdTab==$i){
				$$bancoTem=true;
			}
				$bancoNav []=array('label'=>$this->nav[$i]['nombre'],'url'=>array('admin','bancoIdTab'=>$i,'bancoid'=>$this->nav[$i]['idcaja']),'active'=>$$bancoTem);
			
		};
		 
		 
//--------------------------BARRA DE CAJA CARGADOS-------------------
...
//---------------------------------------------------------------		
	...
	for($i=0;$i<12;$i++){
		...
		$arregloTabsMeses[]=array('label'=>$meses[$i][1],'content'=>$this->renderPartial('gridmovimientocaja', array('model'=>$model,'bancoid'=>$bancoid,'anioTab'=>$anioTab,'mesTab'=>$meses[$i][0],), true),'active'=>$$varTempMes);
		}
	...
	$this->widget('bootstrap.widgets.TbTabs', array(
		...
		echo "<h1> Debe Cargar al menos una caja para comenzar a operar </h1>";
		}
	}
?>
<?php 
